...
const imodsDir =
  (isWebApp && window.webAppIModsDir) ||
  (assetDir ? path.join(assetDir, "archive", "imods") : undefined);
...
function getModStoreKey(mod_id, k) {
  if (k) {
    return `${getModStoreKey(mod_id)}.${k}`;
  }
...
function getAssetRoute(url) {
  ...
  if (!expectWorkingState()) return undefined;
...
function removeModsFromEnabledList(responsible_mods) {
  ...
  const new_enabled_mods = old_enabled_mods
    .filter(x => !responsible_mods.includes(x))
...
function onModLoadFail(responsible_mods, e) {
  if (!expectWorkingState() || flag_failures_dont_interrupt) return; // Pre-setup, we're probably fine ignoring this.
  ...
  if (window.vm && window.vm.loadStage) {
    ...
    <div>
      ...
      <div>
        ...
        <p>For troubleshooting, save this error message or the <a href="${
          log.transports ? log.transports.file.getFile() : ""
        }">log file</a></p><br />
        <p>Stacktrace:</p>
    ...
    try {
      ...
      if (js.trees) {
        console.assert(
          !js._singlefile,
          js.title,
          "Single file mods cannot use treeroute!"
        );
        ...
        for (const mod_tree in js.trees) {
          ...
          console.assert(
            mod_tree.endsWith("/"),
            mod_tree,
            "Tree paths must be directories! (end with /)"
          );
          console.assert(
            asset_tree.endsWith("/"),
            asset_tree,
          ...
          console.assert(
            asset_tree.startsWith("assets://"),
            asset_tree,
            "Asset paths must be on the assets:// protocol!"
          );

          const tree = await crawlFileTree(
            path.join(js._mod_root_dir, mod_tree),
            true
          );
          ...
          treeroutes.forEach(route => {
            ...
            const dest_href = new URL(
              (path.posix || path).join(mod_tree, route),
              js._mod_root_url
            ).href;
            if (route_href == dest_href) {
              console.warn(
                `${js._id} tried to route an asset to itself: ${route_href}`
              );
            } else {
      ...
      for (const key in js.routes || {}) {
        ...
        console.assert(
          !(js._singlefile && local.includes(js._mod_root_url)),
          js.title,
          "Single file mods cannot use local route!"
        );
...
function getEnabledMods() {
  ...
  if (store.get("settings.unpeachy")) list.push("_unpeachy");
  if (store.get("settings.pxsTavros")) list.push("_pxsTavros");
  if (store.get("settings.notitty")) list.push("_notitty");
  if (!store.get("settings.newReader.limit")) list.push("_secret");

  // Complicated flash ones

  if (store.get("settings.jsFlashes")) list.push("_replaybound");

  // Soluslunes must load after bolin
  if (store.get("settings.soluslunes")) list.push("_soluslunes");

  // Bolin must come before hqaudio in the stack so it loads after it.
  if (store.get("settings.bolin")) list.push("_bolin");

  if (store.get("settings.hqAudio")) list.push("_hqAudio");
...
async function getEnabledModsJsAsync(opts) {
  ...
  try {
    const enabled_mod_modules = await Promise.all(
      getEnabledMods().map(async dir => await getModJsAsync(dir, options))
    );
...
async function buildApi(mod) {
  ...
  function safetyChecks(local_path) {
    if (mod._singlefile)
      throw new Error(`Singlefile mods cannot use this method`);
    if (!local_path.startsWith("./"))
      throw new Error(`${local_path}: Paths must be mod relative (./)`);
    if (local_path.includes("/.."))
      throw new Error(`${local_path}: You know what you did`);

    if (isWebApp) {
      console.error(`[DEBUG] safetyChecks failed for mod ${mod._id}:`);
      console.error(`  - Trying to read: ${local_path}`);
      console.error(`  - Mod root dir: ${mod._mod_root_dir}`);
      console.error(`  - isWebApp: ${isWebApp}`);
      console.error(`  - webAppModsDir: ${window.webAppModsDir}`);
      console.error(`  - webAppIModsDir: ${window.webAppIModsDir}`);
      throw new Error(
        `Cannot read files in webapp mode. You need to webpack these! Mod: ${mod._id}, File: ${local_path}`
      );
    }
  }
  ...
  function readFileAsyncLocal(local_path, method_name) {
    ...
    return fs.promises.readFile(
      path.join(mod._mod_root_dir, local_path),
      "utf8"
    );
  }
  ...
  const api = {
    store: {
      ...
      has: k =>
        this_store_cache[k] != undefined ||
        store_mods.has(getModStoreKey(mod._id, k)),
      delete: k => {
        delete this_store_cache[k];
        store_mods.delete(getModStoreKey(mod._id, k));
      },
      onDidChange: (k, cb) =>
        store_mods.onDidChange(getModStoreKey(mod._id, k), cb),
      clear: () => {
        this_store_cache = {};
    ...
    readJsonAsync(asset_path, callback) {
      return readFileAsyncLocal(asset_path, "readJsonAsync")
        .then(text => JSON.parse(text))
        .then(callback);
    },
    readYamlAsync(asset_path, callback) {
      return readFileAsyncLocal(asset_path, "readYamlAsync")
  ...
  try {
    ...
    if (!isWebApp) {
      if (mod_dir.endsWith(".js")) {
        ...
        try {
          const is_directory = !fs
            .lstatSync(path.join(thisModsDir, mod_dir))
            .isFile(); // allow for junctions, symlinks
          if (is_directory) {
            is_singlefile = false;
    ...
    try {
      ...
      if (!is_singlefile) {
        ...
        mod_module._mod_root_url =
          new URL(mod_module._id, thisModsAssetRoot).href + "/";
      }
      ...
      if (!options.liteload) {
        ...
        if (a && b && !semver.gte(a, b)) {
          throw Error(
            `${mod_module._id} ${mod_module.modVersion} requires at least app ${mod_module.minAppVersion}`
          );
        }
        ...
        if (mod_module.asyncComputed != undefined) {
          api = api || (await buildApi(mod_module));
          mod_module._fullyLoadedPromise = mod_module
            .asyncComputed(api)
            .then(result => {
      ...
      if (mod_dir.startsWith("_")) {
        ...
        if (options.noReextractImods) {
          console.log(
            "Already tried to re-extract imods, refusing to infinite loop"
          );
          throw e;
        }
        if (expectWorkingState()) {
        ...
        // eslint-disable-next-line no-undef
        return await getModJsAsync(mod_dir, {
          ...options,
          noReextractImods: true
        });
      } else {
        console.log(
          "Caught error importing non-imod",
          mod_dir,
          modjs_path,
          "; unrecoverable require error"
        );
        throw e;
      }
    }
  ...
  if (js.footnotes) {
    ...
    if (typeof js.footnotes == "string") {
      console.assert(
        !js._singlefile,
        js.title,
        "Single file mods cannot use footnote files!"
      );

      const json_path = path.join(js._mod_root_dir, js.footnotes);

      logger.info(js.title, "Loading footnotes from file", json_path);
      const footObj = JSON.parse(await fs.promises.readFile(json_path));
      ...
      throw new Error(
        `${
          js.title
        } Incorrectly formatted mod. Expected string or array, got '${typeof jsfootnotes}'`
      );
    }
  }
...
async function editArchiveAsync(archive) {
  if (!expectWorkingState()) {
    logger.warn(
      "No asset directory set, probably in new reader setup mode. Not editing the archive."
    );
    return;
  }
        ...
        if (store_mods.has(cache_id)) {
          ...
          importSassJs().then(SassJs => {
            ...
            if (isWebApp) {
              ...
              body = body.replace(/(assets:\/\/.+)(?=\);)/g, match =>
                Resources.resolveAssetsProtocol(match)
              );
            }
        ...
        modules.forEach(async js => {
          ...
          try {
            ...
            modstyles.forEach((customstyle, i) => {
              const style_id = `style-${js._id}-${String(js.version).replace(
                /\./g,
                "_"
              )}-${i}`;
              ...
              if (customstyle.source && customstyle.body) {
                throw Error(
                  "Styles cannot set both source and body. Use multiple style objects."
                );
              }

              if (customstyle.source) {
                const scss_path = path.resolve(
                  js._mod_root_dir,
                  customstyle.source
                );
            ...
            modThemes.forEach((theme, i) => {
              ...
              fs.readFile(scss_path, "utf8", (err, body) => {
                ...
                addScssStyle(
                  theme_class,
                  `#app.${theme_class}, #app > .${theme_class} {\n${body}\n}`
                );
              });
            });
...
async function getMixinsAsync() {
  ...
  var newThemes = mixable_mods.reduce((themes, js) => {
    ...
    return themes.concat(
      js.themes.map((theme, i) => ({
        text: theme.label,
        value: `theme-${js._id}-${i}`
      }))
    );
  }, []);
  if (newThemes) {
      ...
      vueHooks: [
        {
          matchName: "TabFrame",
          data: {
            modBrowserPages($super) {
              return { ...newPages, ...$super };
            }
          },
          created() {
      ...
      vueHooks: [
        {
          ...
          data: {
            browserActions($super) {
              return { ...actionComponents, ...$super };
            }
          }
        }
      ]
  ...
  const mixin = {
    created() {
      ...
      this._uhc_matching_hooks.forEach(hook => {
        ...
        if (hook.created) hook.created.bind(this)();
        ...
        for (const dname in hook.data || {}) {
          ...
          this[dname] =
            typeof value == "function" ? value.bind(this)(this[dname]) : value;
        }
        // Computed
        for (const cname in hook.computed || {}) {
          ...
          try {
            ...
            console.warn(
              "Vue hook: component",
              this.$options.name,
              "does not have existing computed property",
              cname,
              "in",
...
function jsToChoice(js, dir) {
  ...
  if (js.version) {
    logger.warn(
      `${js._id}: the 'version' property is deprecated. Use 'modVersion' and 'minAppVersion' instead.`
    );
  ...
  return {
    ...
    hasmeta: Boolean(
      js.author || js.modVersion || js.settings || js.description
    ),
...
async function loadModChoicesAsync() {
  ...
  const choice_promises = mod_folders.map(async dir => {
    try {
      ...
      if (js === null || js.hidden === true) return false;
...
export default {
  ...
  loadAllContent() {
    Object.values(vm.$archive.mspa.story).forEach(page => page.content);
  },
