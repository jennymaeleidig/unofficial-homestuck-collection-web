...
function getCache(size, funkey) {
  if (funkey == undefined) throw `Function key is undefined!`;

  const exist_cache = all_caches[funkey];
  if (exist_cache) return funkey;

  console.log("new cache for", funkey);
  all_caches[funkey] = new LRUCache(size);
  return funkey;
}
function cacheHas(cache_id, key) {
  return all_caches[cache_id].has(key);
}
function cacheGet(cache_id, key) {
  return all_caches[cache_id].get(key);
}
function cacheSet(cache_id, key, value) {
  all_caches[cache_id].set(key, value);
}
function memoizedClearAll() {
  for (const c in all_caches) all_caches[c].clear();
}
