<template>
  <p
    ...
    :class="fontFamily"
    :style="fontStyle"
    v-html="filteredPrattle"
    v-if="textType == 'prattle' && !!content"
  />

  <div
    class="log"
    :class="{ logHidden: logHidden, darkBackground: isDarkBackground }"
    v-else-if="textType == 'log'"
  >
    <div
      class="bgshade"
      v-if="$localData.settings.textOverride.highContrast"
      :style="{ background: isDarkBackground ? '#000A' : '#FFFA' }"
    />
    <button class="logButton" @click="loggle()">
      {{ logButtonText }}
    </button>
    <p
      class="logContent text"
      :class="fontFamily"
      :style="fontStyle"
  ...
  computed: {
    ...
    fontFamily() {
      ...
      if (this.$localData.settings.textOverride.fontFamily) {
        ...
        if (this.$localData.settings.textOverride.bold) result.push("bold");
      } else {
    ...
    fontStyle() {
      ...
      return {
        ...
        lineHeight:
          lineHeights[this.$localData.settings.textOverride.lineHeight]
      };
    },
    textType() {
    ...
    filterDOM() {
      if (this.$el.nodeType !== 8) {
        ...
        for (let i = 0; i < links.length; i++) {
          ...
          if (
            this.usePurpleLinks &&
            this.$localData.allHistory.includes(filteredLink)
          )
        ...
        for (const image of [...images]) {
          ...
          if (
            ...
            this.$localData.settings.reducedMotion
          ) {
        ...
        if (this.$localData.settings.textOverride.highContrast) {
          this.$el.querySelectorAll("span[style]").forEach(e => {
            ...
            const lightnessExtrema = this.isDarkBackground
              ? 1 - this.lightnessOffset
              : this.lightnessOffset;
            const needClampLightness = this.isDarkBackground
            ...
            const needClampsaturation =
              textcolor.getSaturation() < saturationExtrema &&
              textcolor.getHue();
