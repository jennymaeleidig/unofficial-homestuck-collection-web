<template>
  <GifSeeker
    v-if="mediaType === 'gif' && reduceMotion"
    :src="resourceUrl"
    :noanimate="$localData.settings.reducedMotion && noncritical != undefined"
    class="mediaembed"
  />
  <img
    v-else-if="mediaType === 'img' || mediaType === 'gif'"
    :src="resourceUrl"
  ...
  />
  <video
    v-else-if="mediaType === 'vid' && gifmode != undefined"
    :src="resourceUrl"
    :width="videoWidth"
  ...
  <button
    v-else-if="
      mediaType === 'swf' && reduceMotion && !reducedMotionFlashConfirmed
    "
  ...
  >
    ...
    <canvas
      ...
      :height="
        height || ($localData.settings.jsFlashes && flashProps.id in cropHeight)
          ? cropHeight[flashProps.id]
          : flashProps.height
      "
  ...
  <iframe
    v-else-if="mediaType === 'swf'"
    :key="url"
    :srcdoc="flashSrc"
    :width="width || flashProps.width"
    :height="
      height ||
        ($localData.settings.jsFlashes && flashProps.id in cropHeight
          ? cropHeight[flashProps.id]
          : flashProps.height)
    "
    @load="initIframe"
  ...
  <component
    v-else-if="mediaType === 'html'"
    :is="frameType"
    :src="resolveFrameUrl(url)"
    ref="frame"
    :style="
      `width: ${width || flashProps.width}px; height: ${height ||
        flashProps.height}px; max-width: 100%; max-height: 100%;`
    "
    @did-finish-load="initHtmlFrame"
    seamless
    class="mediaembed"
  />
...
export default {
  ...
  computed: {
    ...
    audioTracks() {
      const ret =
        this.$archive.audioData[this.url.replace("_hq.swf", ".swf")] || [];
      this.$logger.info(
        "Getting audio tracks for",
        this.url,
        this.url.replace("_hq.swf", ".swf"),
        ret
      );
      return ret;
    },
      ...
      return `
        ...
        <script>
          ...
          if (window.parent && window.parent.isWebApp) {
            ...
            // Helper function to resolve URLs through parent's resource system
            function resolveUrl(url) {
              try {
                // Check if this URL should be redirected to local assets
                // Include SWF files and other asset types
                if (url.includes('mspaintadventures.com') ||
                    url.includes('sweetcred.com') ||
                    url.includes('timelesschaos.com') ||
                    url.includes('turner.com/planet/mp3') ||
                    url.includes('fozzy42.com') ||
                    url.includes('pasko.webs.com') ||
                ...
                console.warn("Error resolving URL:", url, e)
              }
              return url
            }
...
document.addEventListener('click', function (e) {
  ...
  if (shouldBlockClick) {
    ...
    initIframe(event) {
      ...
      // For webapp CORS fix, provide access to parent window and resource resolution
      if (this.$isWebApp) {
        target.contentWindow.parent.vm = this;
        target.contentWindow.parent.isWebApp = true;
      }
    },
    resolveFrameUrl(url) {
    ...
    audioStart(n) {
      ...
      if (this.flashProps.id == "04106") {
        if (n.toString().endsWith(".5")) {
          ...
          if (n == "1") cascadeDelay = -52900;
          else if (n == "4") cascadeDelay = -125250;
          else if (n == "5") cascadeDelay = -77900;
        }
      }
      n = parseInt(n) - 1;
      ...
      if (delay > 0) {
        setTimeout(() => {
          if (this.audio[n]) this.audio[n].play();
        }, delay);
      } else {
        if (delay < 0) this.audio[n].currentTime = -0.001 * delay;
        this.audio[n].play();
      }
    },
    audioPause() {
    ...
    audioResume() {
      setTimeout(() => {
        if (this.audio[this.lastStartedAudio])
          this.audio[this.lastStartedAudio].play();
      }, 30);
    },
    audioReset() {
