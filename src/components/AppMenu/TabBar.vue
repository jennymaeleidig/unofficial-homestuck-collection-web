<template>
  <div id="tabBar" v-if="hasTabBarContainer">
    <div class="navigationButtons">
      <button
        class="systemButton historyButton historyBack"
        @click="historyBack"
        @click.middle="historyBackNewTab"
        :disabled="!activeTabHasHistory"
      >
        <fa-icon icon="chevron-left"></fa-icon>
      </button>
      <button
        class="systemButton historyButton historyForward"
        @click="historyForward"
        @click.middle="historyForwardNewTab"
        :disabled="!activeTabHasFuture"
      >
      ...
      <button
        class="systemButton historyButton refresh"
        @click="reloadTab"
    ...
    <template v-if="$localData.settings.showAddressBar">
      ...
      <!-- Toolbars go here -->
      <component
        v-for="(__, componentkey) in browserToolbars"
        :is="componentkey"
        :key="componentkey"
    ...
    <div id="tabSection">
      <template v-if="$localData.settings.useTabbedBrowsing && !$isWebApp">
        <div
          id="dragTab"
          class="tab activeTab"
          tabindex="-1"
        ...
        <button
          class="systemButton newTabButton"
          @click="newTab()"
          title="New tab"
        >
          <span>＋</span>
      ...
      <div class="sysActionButtons">
        <button
          class="systemButton sysActionButton jumpBoxButton"
        ...
        <button
          class="systemButton sysActionButton bookmarksButton"
...
export default {
  ...
  computed: {
    ...
    hasTabBarContainer() {
      ...
      else return this.$localData.settings.showAddressBar;
      // For webapp: only show container if address bar is enabled
      // Internal tabs not supported in webapp mode
    },
    tabs() {
    ...
    thisTabPageId() {
      ...
      if ((match = /^\/mspa\/(\d+)$/.exec(activeTabUrl))) return match[1];
      // eslint-disable-next-line no-cond-assign
  ...
  methods: {
    ...
    getTabEl(el) {
      if (!el) return null;
      return el.classList.contains("tabShell")
        ? el.firstChild
        : el.closest(".tab");
    },

    constrainXToTabArea(tabX) {
      const tabAreaRect = document
        .getElementById("tabs")
    ...
    initDrag(e) {
      ...
      if (
        e.target.classList.contains("closeTabButton") ||
        this.sortedTabList.length <= 1
      )
        return;
    ...
    startDragTab(e) {
      ...
      if (Math.abs(e.clientX - this.clickAnchor) > 5) {
        ...
        this.dragTab.querySelector(
          ".tabTitle"
        ).innerHTML = this.dragTarget.querySelector(".tabTitle").innerHTML;

        if (
          document
            .getElementById("appHeader")
            .classList.contains("headerHoverEnabled")
        )
          document
            .getElementById("appHeader")
            .classList.remove("headerHoverEnabled");

        this.showDragTab = true;
        this.dragTab.style.width = this.dragTarget.clientWidth + "px";
        this.dragTab.style.left =
          this.constrainXToTabArea(
            this.dragTarget.getBoundingClientRect().left + snapDistance
          ) + "px";
        ...
        this.$nextTick(() => {
          const titleWidth =
            this.dragTab.querySelector(".tabTitle").getBoundingClientRect()
              .width - 5; // Offsets 5px of padding on left
          const titleTextWidth = this.dragTab
            .querySelector(".tabTitle span")
    ...
    elementDrag(e) {
      ...
      if (this.thresholdDirection) {
        if (
          (this.thresholdDirection === "left" && e.clientX > this.threshold) ||
          (this.thresholdDirection === "right" && e.clientX < this.threshold)
        ) {
          this.dragTab.style.left =
            this.dragTab.offsetLeft + e.clientX - this.threshold + "px";
      ...
      const xCenter =
        dragTabArea.left + (dragTabArea.right - dragTabArea.left) / 2;
      const yCenter =
        dragTabArea.top + (dragTabArea.bottom - dragTabArea.top) / 2;

      const tabRef = this.getTabEl(document.elementFromPoint(xCenter, yCenter))
        .id;
    ...
    closeDragElement() {
      ...
      if (
        ...
        document
          .getElementById("appHeader")
          .classList.add("headerHoverEnabled");
    ...
    forceReload: function() {
      ...
      if (!this.$isWebApp) {
        ...
        // In webapp mode, just reload the page
