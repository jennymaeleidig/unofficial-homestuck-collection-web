...
function buildChapterIndex(archive) {
  ...
  logger.info(
    "Populating search index with",
    storytextList.length,
    "page documents"
  );
  ...
  const footnoteList = Object.keys(archive.footnotes.story).map(page_num => {
    return {
      ...
      mspa_num: page_num,
      chapter: Resources.getChapter(page_num),
      content: archive.footnotes.story[page_num]
...
async function doSearch(payload) {
  ...
  if (payload == undefined) return; // Just wanted to ensure the index
  ...
  const sortFn = (a, b) => {
    const aKey = Number.isNaN(parseInt(a.key))
      ? keyAlias[a.key]
      : parseInt(a.key);
    const bKey = Number.isNaN(parseInt(b.key))
      ? keyAlias[a.key]
      : parseInt(b.key);
  ...
  for (const page of results) {
    ...
    const page_lines = page.content
      ...
      .map(line => {
        ...
        if (line_text.length > 160) {
          return line_text.split(/(?<=\. )/).map(substr => {
            // non-consuming split
    ...
    const intraResults = intraPageSearch.search(payload.input, {
      limit,
      bool: "or"
    });
    ...
    const spread_indexes = Array.from(
      indexes.reduce((acc, i) => {
        const spread = 2;
        for (
          let j = Math.max(0, i - spread);
          j < Math.min(page_lines.length, i + spread);
          j++
        ) {
          acc.add(j);
        }
