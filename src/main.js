...
Vue.mixin({
  ...
  computed: {
    ...
    $logger() {
      return log.scope(
        this.$options.name || this.$options._componentTag || "undefc!"
      );
    },
    $isWebApp() {
      return window.isWebApp || false;
    }
  },
  methods: {
    $resolvePath(to) {
      ...
      if (!this.$localData.settings.mspaMode && base == "mspa") {
        ...
        if (base == "mspa") {
          ...
          if (p_padded in this.$archive.mspa.story)
            resolvedUrl = `/mspa/${p_padded}`;
    ...
    $openLink(url, auxClick = false) {
      ...
      //
      const re_local = new RegExp(
        `^(http://|https://)?(${regExpEscape(app_domain)}|app:\/\/\\.(index)?)`
      );
      const re_local_index = new RegExp(
        `^(http://|https://)?(${regExpEscape(
          app_domain
        )}|app:\/\/\\.\/)index\\.html\\??`
      );
      ...
      let to =
        /mspaintadventures/.test(urlObject.href) && !!urlObject.search
          ? urlObject.href
          : urlObject.pathname;
      to = to
        .replace(
          /.*mspaintadventures.com\/(\w*\.php)?\?s=(\w*)&p=(\w*)/,
          "/mspa/$3"
        )
      ...
      if (!re_local.test(urlObject.origin)) {
        ...
        this.$logger.error(
          "UNCAUGHT ASSET? Tried to externally open internal(?) asset",
          to
        );
        this.$openModal(to);
      } else if (auxClick) {
        if (window.isWebApp) {
          window.open(window.location.origin + this.$resolvePath(to), "_blank");
        } else {
    ...
    $mspaFileStream(url) {
      ...
      return Resources.toFilePath(
        Resources.resolveURL(url),
        this.$localData.assetDir
      );
    },
    ...
    $vizToMspa(vizStory, vizPage) {
      ...
      if (this.$archive) {
        const pageInStory =
          p in this.$archive.mspa.story || p in this.$archive.mspa.ryanquest;
        if (!pageInStory) {
          this.$logger.info(
            "$vizToMspa: p",
            p,
    ...
    $mspaToViz(mspaInput, isRyanquest = false) {
      ...
      if (this.$archive) {
        const mspaPage = isNaN(mspaInput)
          ? mspaInput
          : mspaInput.padStart(6, "0");
        const pageInStory =
          mspaPage in
          (isRyanquest
            ? this.$archive.mspa.ryanquest
            : this.$archive.mspa.story);
        if (!pageInStory) {
          return undefined;
        }
      }
    ...
    $updateNewReader(thisPageId, forceOverride = false) {
      if (!this.$isNewReader && !forceOverride) return; // don't reset non-new reader back to new-reader mode unless explicitly forced

      const isSetupMode = !this.$archive;
      const isNumericalPage = /\d/.test(thisPageId);
      const endOfHSPage = this.$archive
        ? this.$archive.tweaks.endOfHSPage
        : "010030";
      ...
      if (
        isNumericalPage &&
        isInRange &&
        (isSetupMode || thisPageId in this.$archive.mspa.story)
      ) {
        let nextLimit;
        ...
        if (!isSetupMode) {
          ...
          else if ("006021" <= thisPageId && thisPageId <= "006094")
            nextLimit = "006095";
          // Jane+Jake
          else if ("006369" <= thisPageId && thisPageId <= "006468")
            nextLimit = "006469";
          // Roxy+Dirk
          // A6A5A1x2 COMBO
            ...
            this.$archive.tweaks.tzPasswordPages.includes(thisPageId)
          ) {
          ...
          // On normal pages, always pick the lowest next-pageId available. The higher one is a Terezi password 100% of the time
            ...
            nextLimit = [
              ...this.$archive.mspa.story[thisPageId].next
            ].sort()[0];
        }
        ...
        if (thisPageId == endOfHSPage) {
          ...
          const resultCurrent =
            forceOverride ||
            !this.$newReaderCurrent ||
            this.$newReaderCurrent < thisPageId
              ? thisPageId
              : false;
          const resultLimit =
            forceOverride ||
            !this.$localData.settings.newReader.limit ||
            this.$localData.settings.newReader.limit < nextLimit
              ? nextLimit
              : false;
        ...
        this.$logger.warn(
          `Invalid page ID '${thisPageId}', not updating progress`
        );
    },
    $shouldRetcon(retcon_id) {
      console.assert(
        /retcon\d/.test(retcon_id),
        retcon_id,
        "isn't a retcon ID! Should be something like 'retcon4'"
      );
      // If fast-forward, always retcon.
      if (this.$localData.settings.fastForward) return true;
    ...
    $timestampIsSpoiler(timestamp) {
      ...
      const latestTimestamp = this.$archive.mspa.story[this.$newReaderCurrent]
        .timestamp;
      let nextTimestamp;
      try {
        nextTimestamp = this.$archive.mspa.story[
          this.$archive.mspa.story[this.$newReaderCurrent].next[0]
        ].timestamp;
      } catch {
    ...
    $pageIsSpoiler(page, useLimit = false) {
      ...
      const parsedLimit = parseInt(
        this.$localData.settings.newReader[useLimit ? "limit" : "current"]
      );
      ...
      return (
        ...
        ((page in this.$archive.mspa.story &&
          ...
          page.includes("Hiveswap Friendsim: ") ||
          page.includes("Pesterquest: "))
      );
    },
    $trackIsSpoiler(ref) {
      if (this.$isNewReader && ref in this.$archive.music.tracks) {
        ...
        return !(
          (track.pages &&
            track.pages.find(page => !this.$pageIsSpoiler(page))) ||
          (track.album &&
            track.album.find(album => {
              if (album == "unreleased-tracks" && track.referencedBy) {
                return track.referencedBy.find(
                  track => !this.$trackIsSpoiler(track)
                );
    ...
    $albumIsSpoiler(ref) {
      if (
        this.$isNewReader &&
        ref in this.$archive.music.albums &&
        this.$archive.music.albums[ref].date
      ) {
        // It's a spoiler if it belongs to an album with a more recent timestamp than the current page
        let date;

        if (ref == "homestuck-vol-1")
          date = this.$archive.mspa.story["002340"].timestamp;
        // During third Rose GameFAQs, after Nanna expodump
        else if (ref == "homestuck-vol-5")
          date = this.$archive.mspa.story["003841"].timestamp;
        // Curtains after [S] Descend
        else if (ref == "homestuck-vol-6")
          date = this.$archive.mspa.story["005127"].timestamp;
        // During LE/Recap 3 Huss interruption
        else if (ref == "song-of-skaia")
          date = this.$archive.mspa.story["006291"].timestamp;
        // Immediately after EOA6I1
        else if (ref == "colours-and-mayhem-universe-b")
          date = this.$archive.mspa.story["006716"].timestamp;
        // Immediately after DOTA, before EOY3 shown
        else if (ref == "homestuck-vol-9")
          date = this.$archive.mspa.story["006928"].timestamp;
        // After Terry: FF to Liv, before cherub chess
        else if (ref == "symphony-impossible-to-play")
          date = this.$archive.mspa.story["007162"].timestamp;
        // Just after Caliborn: Enter, before openbound 1
        else if (ref == "one-year-older")
          date = this.$archive.mspa.story["007162"].timestamp;
        // Just after Caliborn: Enter, before openbound 1
        else if (ref == "cherubim")
          date = this.$archive.mspa.story["007882"].timestamp;
        // After Interfishin, right when Caliborn/Calliope expodump begins
        else
          date =
            new Date(this.$archive.music.albums[ref].date).getTime() / 1000;
        this.$logger.debug(ref, this.$archive.mspa.story["006716"].timestamp);
        return (
          date > this.$archive.mspa.story[this.$newReaderCurrent].timestamp
        );
      } else return false;
    }
  }
...
Promise.all(promises_loading).then(_ => {
  ...
  if (
    window.webAppAssetPackHref &&
    !window.webAppAssetPackHref.includes("localhost")
  ) {
    import("@sentry/vue").then(Sentry => {
      Sentry.init({
        app: Vue,
        dsn:
          "https://7e23b32f11076af727a96b6e2961689e@o4505609395306496.ingest.sentry.io/4505609400090624",
        integrations: [
