...
const RE_THIS_DOMAIN = new RegExp(
  `https?://${app_domain.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/`
);
...
function fileIsAsset(url) {
  ...
  const has_file_ext = /\.(jpg|png|gif|swf|txt|mp3|wav|mp4|webm|mov|html)$/i.test(
    url
  );
...
function getResourceURL(request_url) {
  ...
  if (/\.[0-9a-f]{8}\..{3,4}$/.exec(request_url)) {
    logger.error(
      "URL",
      request_url,
      "is webpacked! Replacing would break it. This probably means you're running getResourceURL somewhere you shouldn't!"
    );
  ...
  resource_url = resource_url
    ...
    .replace(
      /.*mspaintadventures.com\/((scratch|trickster|ACT6ACT5ACT1x2COMBO|ACT6ACT6)\.php)?\?s=(\w*)&p=(\w*)/,
      "/mspa/$4"
    ) // Covers for 99% of flashes that link to other pages
    ...
    .replace(
      /.*mspaintadventures.com\/extras\/PS_titlescreen\//,
      "/unlock/PS_titlescreen"
    ) // Link from CD rack flash
    .replace(
      /.*mspaintadventures.com\/sweetbroandhellajeff\/(?:(?:comoc\.php)?\?cid=0(\d{2})\.jpg)?/,
      "/sbahj/$1"
    )
    .replace(
      /^http(s{0,1}):\/\/www\.sweetcred\.com/,
      `assets://archive/sweetcred`
    )
    .replace(
      /^http(s{0,1}):\/\/www\.timelesschaos\.com\/transferFiles/,
      `assets://storyfiles/hs2/03318`
    ) // return to core - 618heircut.mp3
    .replace(
      /^http(s{0,1}):\/\/(www\.turner\.com\/planet\/mp3|fozzy42\.com\/SoundClips\/Themes\/Movies|pasko\.webs\.com\/foreign)/,
      `assets://storyfiles/hs2/00338`
    ) // phat beat machine
    .replace(
      /^http(s{0,1}):\/\/www.whatpumpkin\.com\/squiddles\.htm(l)?/,
      "/squiddles/credits"
    )
  ...
  if (fileIsAsset(resource_url)) {
    resource_url = resource_url
      .replace(
        /^http(s{0,1}):\/\/((www|cdn)\.)?mspaintadventures\.com\//,
        "assets://"
      )
      .replace(/^\//, "assets://")
      .replace(/^\\/, "assets://")
    ...
    resource_url = resource_url.replace(
      /^http(s{0,1}):\/\/((www|cdn)\.)?mspaintadventures\.com\/storyfiles\/hs2\/waywardvagabond/,
      "/waywardvagabond"
    );
...
function resolveAssetsProtocol(asset_url, loopcheck) {
  ...
  console.assert(
    ...
    "resources",
    asset_url + " is not on the assets protocol!"
  );
  ...
  if (Mods) {
    ...
    if (mod_route) {
      ...
      if (loopcheck.includes(mod_route)) {
        ...
        console.error(
          Object.fromEntries(loopcheck.map(k => [k, Mods.getAssetRoute(k)]))
        );
  ...
  if (assets_root == undefined) {
    logger.error(
      "Asked to resolve assets protocol before resources initialized!",
      asset_url
    );
...
const UrlFilterMixin = {
  methods: {
    filterLinksAndImages(el) {
      let ipcRenderer;
      if (!isWebApp) {
        ipcRenderer = require("IpcRenderer");
      }
      ...
      for (let i = 0; i < media.length; i++) {
        ...
        if (!isWebApp && media[i].tagName == "IMG" && !media[i].ondragstart) {
          media[i].ondragstart = e => {
            ...
            if (!window.isWebApp && ipcRenderer) {
...
const all_stories = ["1", "2", "3", "4", "5", "6", "ryanquest", "snaps"];
...
function getStoryNum(pageNumber) {
  ...
  // JAILBREAK
  if (pageNumber <= 135 || pageNumber == "jb2_000000") return 1;
  // BARD QUEST
  else if (pageNumber >= 136 && pageNumber <= 216) return 2;
  // BLOOD SPADE
  else if (pageNumber == "mc0001") return 3;
  // PROBLEM SLEUTH
  else if (pageNumber >= 219 && pageNumber <= 1892) return 4;
  // HOMESTUCK BETA
  else if (pageNumber >= 1893 && pageNumber <= 1900) return 5;
  // HOMESTUCK
  else if (
    (pageNumber >= 1901 && pageNumber <= 10030) ||
    pageNumber == "pony" ||
    pageNumber == "pony2" ||
    pageNumber == "darkcage" ||
    pageNumber == "darkcage2"
  )
    return 6;
...
function vizToMspa(vizStory, vizPage) {
  ...
  const storyNum =
    vizStory == "ryanquest" ? "ryanquest" : getStoryNum(mspaPage);
  if (!storyNum) {
...
function mspaToViz(mspaInput, isRyanquest = false) {
  ...
  if (isRyanquest) {
    ...
    switch (mspaStory) {
      ...
      case 6:
        vizStory = "homestuck";
        vizPage = isNaN(mspaPage)
          ? mspaPage
          : (parseInt(mspaPage) - 1900).toString();
        break;
    }
  ...
  if (!p) {
    switch (key) {
      ...
      case "MC00001":
        return "Blood Spade";
      case "pony":
        return "Homestuck Act 3";
      case "darkcage":
        return "Homestuck Act 6 Intermission 1";
      case "pony2":
        return "Homestuck Act 2";
      case "darkcage2":
        return "Homestuck Act 6 Act 3";
    }
...
module.exports = {
  init(settings) {
    ...
    if (!isWebApp) {
      ...
      ipcRenderer.on(
        "RESOURCES_RESOLVE_ASSETS_PROTOCOL",
        (event, reply_channel, url) => {
