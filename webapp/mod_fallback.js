...
function mapToVizCanonical(cdn_domain, media_url) {
  ...
  ret = tryMapAndReplace(
    u.pathname,
    /^(?:.*AssetPackV2Lite)\/storyfiles\/(.+(?:\.gif|\.png|\.jpg))/,
    "/images/storyfiles/$1"
  );
  if (ret) return cdn_domain + ret.slice(1);
  ret = tryMapAndReplace(
    u.pathname,
    /^(?:.*AssetPackV2Lite)\/ryanquest\//,
    "/images/ryanquest/"
  );
  if (ret) return cdn_domain + ret.slice(1);
  ret = tryMapAndReplace(
    u.pathname,
    /^(?:.*AssetPackV2Lite)\/advimgs\//,
    "/images/advimgs/"
  );
  if (ret) return cdn_domain + ret.slice(1);
  ret = tryMapAndReplace(
    u.pathname,
    /^(?:.*AssetPackV2Lite)\/scraps\//,
    "/images/scraps/"
  );
  if (ret) return cdn_domain + ret.slice(1);
  ret = tryMapAndReplace(
    u.pathname,
    /^(?:.*AssetPackV2Lite)\/storyfiles\/hs2\/waywardvagabond\//,
    "/images/storyfiles/hs2/waywardvagabond/"
  );
  if (ret) return cdn_domain + ret.slice(1);
}
...
function fallbacksFor(src_orig) {
  ...
  if (supplemental_src) {
    ...
    ret.push(
      mapToVizCanonical("https://file.garden/ZMykmy0EPHK4dlCG/", src_orig)
    );
    ret.push(mapToVizCanonical("https://homestuck.kici.moe/", src_orig));
    ret.push(
      mapToVizCanonical("https://mspa.chadthundercock.com/", src_orig).replace(
        "/images/",
        "/mspa/"
      )
    );
