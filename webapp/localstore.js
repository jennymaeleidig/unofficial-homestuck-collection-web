...
store = {
  ...
  get(key) {
    if (key == undefined) {
      ...
      const parsed = Object.fromEntries(
        Object.entries(localStorage).map(t => {
          try {
      ...
      const value =
        json_repr == "undefined" ? undefined : JSON.parse(json_repr);
  ...
  migrate(migrations, versionToMigrate) {
    ...
    function _shouldPerformMigration(
      candidateVersion,
    ...
    for (const version of newerVersions) {
      try {
        ...
        console.error(
          `Something went wrong during the migration! ${version} ${error}`
        );
      }
    }
    if (
