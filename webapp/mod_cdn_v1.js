...
const asset_pack_supplemental_href =
  "https://file.garden/YOVpTKX47HhECOuA/AssetPack";

const routes = {
  "assets://archive/tbiy/The%20Baby%20Is%20You%20-%20Complete.mp3":
    asset_pack_supplemental_href + "/The%20Baby%20Is%20You%20-%20Complete.mp3",
  "assets://archive/external/howdoilive.mp4":
    asset_pack_supplemental_href + "/external/howdoilive.mp4",
  "assets://archive/external/rufio.mp4":
    asset_pack_supplemental_href + "/external/rufio.mp4",
  "assets://archive/external/midnightcrew.mp4":
    asset_pack_supplemental_href + "/external/midnightcrew.mp4",
  "assets://archive/external/bunny.mp4":
    asset_pack_supplemental_href + "/external/bunny.mp4",
  "assets://archive/wizardyherbert/WH.epub":
    asset_pack_supplemental_href + "/archive/wizardyherbert/WH.epub",
  "assets://images/trickster_sitegraphics/menu.swf":
    asset_pack_supplemental_href + "/archive/trickster_menu.swf",
  "assets://extras/PS_titlescreen/PS_titlescreen.swf":
    asset_pack_supplemental_href + "/PS_titlescreen.swf"
};
...
function cdnify(media_url) {
  ...
  if (cdn_domain) {
    mapped = media_url
      .replace(
        /^assets:\/\/storyfiles\/(.+)\/Sfiles\/(.+?)\/\2/,
        asset_pack_supplemental_href + "/$1/$2/$3"
      )
      .replace(
        /^assets:\/\/storyfiles\/(.+)\/(.+?)\/\2([0-9a-z_]+)?(\.swf|\.mp4|\.mp3)/,
        asset_pack_supplemental_href + "/$1/$2/$2$3$4"
      )
      .replace(
        /^assets:\/\/storyfiles\/hs2\/(06276\/A6A6I1\.swf)/,
        asset_pack_supplemental_href + "/$1"
      )
      .replace(
        /^assets:\/\/storyfiles\/(.+(?:\.gif|\.png|\.jpg))/,
        cdn_domain + "images/storyfiles/$1"
      )
      ...
      .replace(
        /^assets:\/\/storyfiles\/hs2\/waywardvagabond\//,
        cdn_domain + "images/storyfiles/hs2/waywardvagabond/"
      );
  }

  console.assert(
    !(mapped.includes(".swf") && mapped.includes(cdn_domain)),
    mapped,
    media_url
  );
  // if (media_url == mapped) {
...
module.exports = {
  ...
  edit(archive) {
    ...
    replaceContent(
      archive.mspa.story,
      "004718",
      '<a href="/archive/external/miracles.mp4"',
      '<a href="https://www.youtube.com/watch?v=8GyVx28R9-s"'
    );

    archive.mspa.story["004692"].media = [
      "https://mrcheeze.github.io/homestuck-ruffle-tester/flash/02791_local.swf"
    ];
